/**
 * Curis by Citrus - Patient Terms and Conditions JavaScript
 * Provides interactive functionality and enhanced user experience
 * for the Terms and Conditions page in the Curis platform.
 * 
...
function setupTableOfContents() {
    ...
    if (mainSections.length >= 3) {
        ...
        // Insert TOC after the terms header
        const article = document.querySelector('main > article');
        const termsHeader = article.querySelector('.terms-header');

        if (termsHeader) {
            article.insertBefore(tocContainer, termsHeader.nextSibling);
        } else {
        ...
        entries.forEach(entry => {
            ...
            if (tocLink) {
                if (entry.isIntersecting) {
                    ...
                    if (entry.target.parentElement.classList.contains('terms-and-conditions')) {
...
function setupAccessibilityFeatures() {
    ...
    // Add button for printing the terms
    const printButton = document.createElement('button');
    printButton.className = 'print-terms';
 ...
 * Calculates and displays an estimated reading time for the terms
 */
function addReadingTimeEstimate() {
    // Get all text content from the terms and conditions
    const termsContent = document.querySelector('.terms-and-conditions').textContent;

    // Count words (rough approximation: split by spaces)
    const wordCount = termsContent.split(/\s+/).length;
    ...
    // Add to the top of the terms, after the terms metadata
    const termsMetadata = document.querySelector('.terms-metadata');
    if (termsMetadata) {
        termsMetadata.parentNode.insertBefore(readingTime, termsMetadata.nextSibling);
    }
}
...
function setupPrintFunctionality() {
    ...
    window.addEventListener('beforeprint', function () {
        ...
        // Hide print button
        const printButton = document.querySelector('.print-terms');
        if (printButton) printButton.style.display = 'none';

        // Hide agreement checkbox
        const agreementCheckbox = document.querySelector('.agreement-checkbox');
        if (agreementCheckbox) agreementCheckbox.style.display = 'none';
    ...
    window.addEventListener('afterprint', function () {
        ...
        const agreementCheckbox = document.querySelector('.agreement-checkbox');
        if (agreementCheckbox) agreementCheckbox.style.display = '';
...
/**
 * Agreement Checkbox
 * Adds a checkbox for users to confirm they've read the terms
 */
function setupAgreementCheckbox() {
    // Create agreement checkbox element
    const agreementContainer = document.createElement('div');
    agreementContainer.className = 'agreement-checkbox';

    const checkbox = document.createElement('input');
    checkbox.type = 'checkbox';
    checkbox.id = 'agree-terms';
    checkbox.name = 'agree-terms';

    const label = document.createElement('label');
    label.htmlFor = 'agree-terms';
    label.textContent = 'I have read and agree to these Terms and Conditions';
    ...
    if (contact) {
        contact.parentNode.insertBefore(agreementContainer, contact.nextSibling);
    } else {
        document.querySelector('main > article').appendChild(agreementContainer);
    }

    // Store agreement status in sessionStorage
 ...
 * Displays a message when the user agrees to the terms
 */
function showAgreementConfirmation() {
    // Create confirmation element
    const confirmation = document.createElement('div');
    confirmation.className = 'agreement-confirmation';
    confirmation.innerHTML = '<i class="fas fa-check-circle"></i> Thank you for agreeing to our terms.';
...
/**
 * Search Highlight
 * Allows highlighting terms when arriving from a search
 */
function setupSearchHighlight() {
    ...
    if (searchTerm) {
        // Highlight the term throughout the document
...
function highlightSearchTerm(term) {
    const textNodes = getTextNodes(document.querySelector('.terms-and-conditions'));
    ...
    // If we found any matches, add a clear button
    if (document.querySelector('.search-highlight')) {
        const clearButton = document.createElement('button');
        clearButton.className = 'clear-highlights';
        clearButton.innerHTML = 'Clear highlights';
        clearButton.addEventListener('click', clearHighlights);

        // Add to top of document, after the TOC
        const toc = document.querySelector('.toc');
        if (toc) {
            toc.parentNode.insertBefore(clearButton, toc.nextSibling);
        } else {
            const termsHeader = document.querySelector('.terms-header');
            if (termsHeader) {
                termsHeader.parentNode.insertBefore(clearButton, termsHeader.nextSibling);
            }
        }
    }
}
...
dynamicStyles.textContent = `
    ...
    .agreement-confirmation i {
        margin-right: var(--spacing-sm);
    }
    
    .agreement-confirmation.hide {
    ...
    .print-terms:hover, .print-terms:focus {
        background-color: var(--primary-navy);
    }
    ...
    .clear-highlights:hover, .clear-highlights:focus {
